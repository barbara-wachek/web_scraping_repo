...
def dictionary_of_article_from_html(article_link, articel_html):
    ...
    if soup.find('div', class_='article hentry zapiski'):
        article_body = soup.find('div', class_='article hentry zapiski')
    elif soup.find('div', class_='article hentry'):
        article_body = soup.find('div', class_='article hentry')
    else: return
...
#%%
sitemap_url = 'https://pamiec-smieny.blogspot.com/sitemap.xml'
...
with open(f'data/pamiec_smieny_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
...
with pd.ExcelWriter(f"data/pamiec_smieny_{datetime.today().date()}.xlsx", engine='xlsxwriter', options={'strings_to_urls': False}) as writer:    
...
upload_file_list = [f"data/pamiec_smieny_{datetime.today().date()}.xlsx", f'data/pamiec_smieny_{datetime.today().date()}.json']
