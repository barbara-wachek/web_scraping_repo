...
def get_article_links():     
    for number in range(1, 18):
        format_link = f'https://www.wforma.eu/planeta-ewy-sonnenberg,2948,,{number}.html'
        link = format_link 
...
df.to_json(f'data/wforma_planeta_ewy_sonnenberg_{datetime.today().date()}.json', orient='records', force_ascii=False, indent=2)

with pd.ExcelWriter(f"data/wforma_planeta_ewy_sonnenberg_{datetime.today().date()}.xlsx", engine='xlsxwriter') as writer:    
