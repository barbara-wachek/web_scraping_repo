...
def get_article_links():     
    for number in range(1, 18):
        format_link = f'https://www.wforma.eu/bazgroly-boguslawa-kierca,4203,,{number}.html'
        link = format_link 
...
df.to_json(f'data/wforma_bazgroly_boguslawa_kierca_{datetime.today().date()}.json', orient='records', force_ascii=False, indent=2)

with pd.ExcelWriter(f"data/wforma_bazgroly_boguslawa_kierca_{datetime.today().date()}.xlsx", engine='xlsxwriter') as writer:    
