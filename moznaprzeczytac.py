...
def get_links_of_sitemap(sitemap_link):
    # sitemap_link = 'https://moznaprzeczytac.pl/sitemap_index.xml'
    html_text = requests.get(sitemap_link).text
    soup = BeautifulSoup(html_text, "html.parser")
    post_sitemap_links = [x.text for x in soup.find_all('loc') if 'post-sitemap' in x.text]
    links = []
    for l in post_sitemap_links:
        html_text = requests.get(l).text
        soup = BeautifulSoup(html_text, "html.parser")
        post_links = [x.text for x in soup.find_all('loc') if x.text.count('/') == 4]
...
#%% main
articles_links = get_links_of_sitemap('https://moznaprzeczytac.pl/sitemap_index.xml')
