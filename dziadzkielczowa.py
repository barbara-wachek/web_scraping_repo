...
def dictionary_of_article(article_link):
    article_link = 'https://dziad-z-kielczowa.blogspot.com/2021/03/wirydarz-dla-przyjaciela-3-swiadkowie.html'
    ...
    date_of_publication = soup.find('time').text
    
    #spróbować wyjąc name datetime albo przekszalcic text regeksem
    ...
    date_of_publication = re.sub(r'^.* \d{1,2}, \d{4}$', , date_of_publication)
...
with open(f'dziadzkielczowa_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
...
with pd.ExcelWriter(f"dziadzkielczowa__{datetime.today().date()}.xlsx", engine='xlsxwriter', options={'strings_to_urls': False}) as writer:    
