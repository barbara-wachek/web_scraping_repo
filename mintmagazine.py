...
def get_article_links(link):
    ...
    # link = 'https://mintmagazine.pl/literatura?sortuj=najnowsze&strona=4&partial=true'
    ...
    links = ["https://mintmagazine.pl/" + x.a.get('href') for x in soup.find_all('div', class_=re.compile(r'tile-image')) if x.a != None]
    
    article_links = list(set([x for x in links if re.match(r'https\:\/\/mintmagazine\.pl\/\/artykuly\/.*', x)])) 
...
#%% main
article_links = get_article_links('https://mintmagazine.pl/literatura?sortuj=najnowsze&strona=4&partial=true')
