...
def get_links_from_archive(link):
    ...
    #Do zrobienia - czasami w polu Tekst artykulu jest pusto, bo autorka stosuje inne style i wtedy tego nie odnajduje
    
    
def dictionary_of_article(article_link):
    # article_link = 'https://godsavethebook.pl/2018/11/gra-w-klasy-nie-ma-mariusz-szczygiel.html'
    # article_link = 'https://godsavethebook.pl/2019/02/spor-o-pamiec-my-z-jedwabnego-anna-bikont.html'
    
    # article_link = 'https://godsavethebook.pl/2011/05/granica-niezrozumienia-obwod-gowy.html'
    # article_link = 'https://godsavethebook.pl/2017/01/tozsamosc-i-pec-middlesex-jeffrey.html'
    article_link = 'https://godsavethebook.pl/2016/12/zyczenia.html'
    ...
    categories = [x.text.strip() for x in soup.find_all('span', class_='bl_categ')]
    article = soup.find('div', class_='entry-content')
    text_of_article = "\n".join([e.text for e in article.find_all('p')])
    
    if text_of_article == '' or ' ':
...
with open(f'data\\godsavethebooks_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
...
with pd.ExcelWriter(f"data\\godsavethebooks_{datetime.today().date()}.xlsx", engine='xlsxwriter', options={'strings_to_urls': False}) as writer:    
