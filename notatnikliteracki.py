...
Original file is located at
    https://colab.research.google.com/drive/1Sw-0taEfsLXZeWTRGiNwVpYMMnczcezT
"""
...
def get_all_links(first_article_link):
  html_text = requests.get(first_article_link).text
  soup = BeautifulSoup(html_text, "html.parser")
...
def dictionary_of_article(article_link):
    ...
    try:
      date_of_publication = soup.find('span', class_='text text--uppercase text--s-paragraph-12 text--gray').text.strip()
      date_of_publication = date_change_format_short(date_of_publication)
    except:
      date_of_publication = "no date"
    raw_text = soup.find("div", class_= "entry-content")
    try:
        text = soup.find("div", class_= "entry-content").text.strip().replace('\n', ' ').replace('\xa0', ' ')
        author = soup.find('span', class_='text text--s-category text--uppercase').text.strip()
        title = soup.find('h1', class_='text text--s-h1').text
...
sitemap_links = get_sitemap_links("https://notatnikliteracki.pl/post-sitemap.xml")
...
with open(f'notatnikliteracki_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, default=str)
df = pd.DataFrame(all_results)
with pd.ExcelWriter(f"notatnikliteracki_{datetime.today().date()}.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
