...
def dictionary_of_article(article_link):
    ...
    try:
        title_of_article = soup.head.title.text
    except (AttributeError, KeyError, IndexError):
        title_of_article = ""
...
#%%
sitemap_links = [f'https://www.prowincjonalnanauczycielka.pl/sitemap.xml?page={e}' for e in range(1,8)]
...
#%%
with open(f'data/prowincjonalnanauczycielka_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, default=str)
df = pd.DataFrame(all_results)
df["Data publikacji"] = pd.to_datetime(df["Data publikacji"]).dt.date
df = df.sort_values('Data publikacji', ascending=False)
with pd.ExcelWriter(f"data/prowincjonalnanauczycielka_{datetime.today().date()}.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
