...
def dictionary_of_article(article_link):
    ...
    try:
        ...
        if not any([e in category.text for e in ['Literatura', 'Książki', 'Film']]):
            return
    except:
        return

    try:
      date_of_publication = soup.find('span', class_='date').text
      date_of_publication = date_change_format_short(date_of_publication)
    except:
      date_of_publication = ""

    try:
        text_of_article = soup.find('div', class_='entry')
    ...
    try:
        title_of_article = soup.title.text.replace('Netkultura.pl – ', '')
        title_of_article = title_of_article.split(':')
        if len(title_of_article) > 1:
            title_of_article = title_of_article[1].strip()
        else:
            title_of_article = title_of_article[0].strip()
    except (AttributeError, KeyError, IndexError):
        title_of_article = ""

    try:
        author = soup.title.text.replace('Netkultura.pl – ', '').split(':')
        if len(author) > 1:
...
#%%
sitemap_links = get_sitemap_links("https://www.netkultura.pl/wp-sitemap-posts-post-1.xml")
all_results = []
errors = []
...
#%%
with open(f'data/netkultura_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, default=str)
df = pd.DataFrame(all_results)
df["Data publikacji"] = pd.to_datetime(df["Data publikacji"]).dt.date
df = df.sort_values('Data publikacji', ascending=False)
with pd.ExcelWriter(f"data/netkultura_{datetime.today().date()}.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
