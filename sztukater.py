...
def get_article_links(url):
    ...
    links = soup.find_all(lambda tag: tag.name == "a" and tag.text == 'Więcej')
    links = [a['href'] for a in links]
    return links

def dictionary_of_article(article_link):
    article_link = 'https://sztukater.pl' + article_link
    ...
    # texts
    text_divs = soup.find_all('div', {'data-rlta-element': 'panel'})

    records = []
    for article_div in text_divs:

        author_label = article_div['id'].replace('rlta-panel-', '')
        direct_article_url = article_link + '#' + author_label

        try:
            author = soup.find('div', {'id': f'rlta-{author_label}'}).text.strip()
        except (AttributeError, KeyError, IndexError):
            author = ""

        try:
            text_of_article = article_div
            article = article_div.text.strip().replace('\n', ' ').replace('\xa0', ' ')
        except (AttributeError, KeyError, IndexError):
            article = ""

        try:
            title_of_article = soup.head.title.text.replace(' - sztukater.pl', '')
        except (AttributeError, KeyError, IndexError):
            title_of_article = ""
        
        try:
            related_author = soup.find(lambda tag: tag.name == "div" and tag.text.startswith('        Autor:')).text.replace('Autor:', '').strip()
        except (AttributeError, KeyError, IndexError):
            related_author = ""
        ...
        dictionary_of_article = {"Link": direct_article_url,
                                "Data publikacji": date_of_publication,
                                "Tytuł artykułu": title_of_article,
                                "Tekst artykułu": article,
                                "Autor": author,
                                "Utwór powiązany": related_author + ' - ' + title_of_article,
                                "Kategoria": "recenzja",
                                "Tagi": tags,
        ...
        records.append(dictionary_of_article)

    return records

#%%
base_url = 'https://sztukater.pl/ksiazki.html?start='

article_links = []
article_links_errors = []
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = [executor.submit(get_article_links, base_url + str(page)) for page in range(0, 29360, 10)]
...
all_results = [rec for sublist in all_results for rec in sublist]

#%%
with open(f'data/sztukater_{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, default=str)
df = pd.DataFrame(all_results)
df["Data publikacji"] = pd.to_datetime(df["Data publikacji"]).dt.date
df = df.sort_values('Data publikacji', ascending=False)
with pd.ExcelWriter(f"data/sztukater_{datetime.today().date()}.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
