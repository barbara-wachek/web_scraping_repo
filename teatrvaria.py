...
Original file is located at
    https://colab.research.google.com/drive/1vpjCdRhxpve8HV7XALjEd1a2-w5p_Gxq
"""
...
def date_change_format_long(date_of_publication):
    # Dopasowanie do formatu "października 12, 2002"
    date = re.sub(r'(.*?)(\s\d{1,2},\s)(\d{4})', r'\1 \2\3', date_of_publication).strip()

    # Tabela zamieniająca nazwy miesięcy na ich reprezentację liczbową
    lookup_table = {
    ...
    # Konwersja daty na format "%m %d, %Y"
    result = strptime(date, "%m %d, %Y")
    changed_date = datetime.fromtimestamp(mktime(result))
    new_date = changed_date.strftime("%Y-%m-%d")
...
def dictionary_of_article(article_link):
    ...
    soup = BeautifulSoup(html_text, 'lxml')
    try:
      date_of_publication = soup.find('div', class_='post-header').text.strip()
    ...
    text_of_article = soup.find('div', class_='post-body entry-content float-container')
    if text_of_article != None:
        article = text_of_article.text.strip()
...
sitemap_links = get_sitemap_links("https://teatrvaria.blogspot.com/sitemap.xml")
all_results = []
errors = []
with ThreadPoolExecutor() as excecutor:
    list(tqdm(excecutor.map(dictionary_of_article, sitemap_links),total=len(sitemap_links)))
with open(f'teatrvaria{datetime.today().date()}.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, default=str)
df = pd.DataFrame(all_results)
df["Data publikacji"] = pd.to_datetime(df["Data publikacji"]).dt.date
df = df.sort_values('Data publikacji', ascending=False)
with pd.ExcelWriter(f"teatrvaria{datetime.today().date()}.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
